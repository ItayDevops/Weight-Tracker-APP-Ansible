resource "azurerm_availability_set" "avset" {
   name                         = "avset"
   location                     = azurerm_resource_group.Weight-Tracker-App-stage.location
   resource_group_name          = azurerm_resource_group.Weight-Tracker-App-stage.name
 }
######Managed-Db-Postgres-prod########
resource "azurerm_postgresql_server" "VM-DB-SERVER" {
  name                = "postgres-sql-stage"
  location            = azurerm_resource_group.Weight-Tracker-App-stage.location
  resource_group_name = azurerm_resource_group.Weight-Tracker-App-stage.name
